			...
			<div class="loginDiv widgetHeight">
				...
				<div id="loginFormDiv">
					<form class="form-horizontal" method="POST" action="index.php">
						...
						<div class="group">
							<button type="submit" class="button buttonBlue">登录</button><br>
							<a class="forgotPasswordLink" style="color: #15c;">忘记密码了?</a>
						</div>
					</form>
				</div>
				...
				<div id="forgotPasswordDiv" class="hide">
					<form class="form-horizontal" action="forgotPassword.php" method="POST">
						...
						<div class="group">
							...
							<span>请输入详细信息再提交<a class="forgotPasswordLink pull-right" style="color: #15c;">返回</a></span>
						</div>
					</form>
				</div>
			</div>
			...
			jQuery(document).ready(function () {
				...
				forgotPasswordDiv.find('button').on('click', function () {
					...
					if (username === '') {
						...
						errorMessage = '请输入有效的 Email 地址';
						result = false;
					} else if (email.match(illegalChars)) {
						errorMessage = 'Email 地址包含非法字符。';
						result = false;
					}
					if (errorMessage) {
						validationMessage.removeClass('hide').text(errorMessage);
